...
import ru.netology.dto.PostRepost
...
class PostRepostHolder(itemView: View) : PostHolder(itemView) {
    ...
    override fun bind(post: Post) {
        ...
        if (post is PostRepost) {
            val repost = post.reposted

            val ago = (curentDate.time - repost.created.time) / 1_000
            createdTvRepost.text = verboseTime(ago)
            authorTvRepost.text = repost.author
            val avatarAithorRepost = itemView.avatarIvRepost
            itemView.contentTvRepost.text = repost.content
            Glide.with(itemView.context).applyDefaultRequestOptions(requestOptions)
                .load(repost.avatarurl).into(avatarAithorRepost)
